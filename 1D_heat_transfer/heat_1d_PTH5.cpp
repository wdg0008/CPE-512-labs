...
//                             (nthreads < active_points)
//                             Block Distribution of active points
//                             between threads
//                             Only One Worker Thread Generation Phase
//                             which occurs at the beginning of the
//                             compute_temp function
// g++ heat_1d_PTH3.cpp -o heat_1d_PTH3 -lpthread
#include <stdlib.h>
#include <stdio.h>
#include <iostream>
#include <iomanip>
...
int nthreads,active_points,num_iterations;
pthread_barrier_t BARRIER;

//  Each thread computes one contiguous block of active points.
void *thread_total(void * arg) {
    int tid=*(int *) arg;
    int local_block_points = active_points / nthreads;
    int local_extra_points = active_points % nthreads;

    // each prior thread could have been assigned one of the leftovers
    // but once you run out of remainder, all are normal size
    int extra_before = min(tid, local_extra_points);

    int start = (tid * local_block_points) + 1 // for temp[0] boundary
        + extra_before;

    // early threads own an extra point from the remainder
    int local_points = local_block_points + (tid < local_extra_points);

    int stop = start + local_points;
...
void compute_temp(double * temp, int active_points, int num_iterations) {
    ...
    auto threads = make_unique<pthread_t[]>(nthreads);

    // dynamically allocate ids[nthreads] on heap 
    // int *ids = new int [nthreads];
    auto ids = make_unique<int[]>(nthreads);

    // Load zero-based logical thread IDs for block assignment.
