// Travelling Salesman Program -- pthread Version 1
// (base pthread version -- with a potential race condition)
// Unmitigated Distributed Access to Best City Tour shared data Version
//
// B. Earl Wells -- September 2025
//
// Assignment: Modify code so that shared variables best_city_visit_order,
//             and best_cost can only be updated in the city_tours_gen() 
//             function by a single thread at a time. Do this through
//             the use of a MUTEX lock
//
...
void city_tours_gen(int city_order_slot, int city_visit_order[],bool city_msk[],
   ...
   // if there are no more slots available, then city visit order is complete
      ...
      // if current tour cost is less t:han the best cost so far
      // overwrite the current best cost and best city visit order
      // global data
      //
      // Protect the comparison of the best cost variable with the
      // cummulative cost of the current tour variable and the
      // overwriting of the city_visit_order array using
      // a pthread MUTEX lock and unlock at the
