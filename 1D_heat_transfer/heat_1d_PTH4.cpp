...
//  combined thread that takes on an equal number of active
//  point to compute
void *thread_total(void * arg) {
    int local_block_points = active_points / nthreads;
    int local_extra_points = active_points % nthreads;
    assert(local_block_points > 0 && local_extra_points == 0
        && "active_points must be a multiple of nthreads"); // weird string hack

    int tid=*(int *) arg;
    int start = tid * local_block_points + 1;
    int stop = start + local_block_points;
