// 1-D temperature Example  -- Fine-grain pthread Implementation
//                             Compute and copy thread per active element
...
// Create new element -- put in temp_buf array 
...
void compute_temp(double * temp, int active_points, int num_iterations) {
    ...
    // dynamically allocate threads[active_points] on heap
    pthread_t *threads = new pthread_t [active_points]; 

    // dynamically allocate ids[active_points] on heap 
    int *ids = new int [active_points];

    // load logical id structure array -- start numbering at 1
    for (int tid=0; tid<active_points; tid++) {
       ids[tid]=tid+1;
    }
    for (int i=0;i<num_iterations;i++) {
        ...
        // Thread Create SUBPHASE -- one thread for each element
        for (int tid=0; tid<active_points; tid++) {
...
void print_usage_instructions(char *command) {
    cout << "Usage: " << command << 
        ...
        "<x>" << endl <<
        "   where where optional argument x = " << endl <<
